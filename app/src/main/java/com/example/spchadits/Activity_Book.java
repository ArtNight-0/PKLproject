...
//        });
    ...
    private void getContent() {
        ...
        try {
            ...
            Call call = uploadApis.getContentBook();
            call.enqueue(new Callback() {
                @Override
                public void onResponse(Call call, Response response) {
                    if (response.code() == 200) {
                        HaditsBook data = (HaditsBook)response.body();
                        //String gson = new Gson().toJson(response.body());

                        mAdapter = new Adapter_Book(data);
                        mRecyclerView.setAdapter(mAdapter);

                        String message = "";
                    } else {
                        Toast.makeText(Activity_Book.this, response.message(), Toast.LENGTH_LONG).show();
                    }
                }
